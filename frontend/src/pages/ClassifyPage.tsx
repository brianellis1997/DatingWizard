                    ...
                    {!result.user_feedback ? (
                      <div className="flex gap-3">
                        <button
                          ...
                          className="flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg transition-colors bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-red-100 dark:hover:bg-red-900/30 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                        ...
                        <button
                          ...
                          disabled={feedbackMutation.isPending}
                          className="flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg transition-colors bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-green-100 dark:hover:bg-green-900/30 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                        ...
                        <button
                          ...
                          disabled={feedbackMutation.isPending}
                          className="flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg transition-colors bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-yellow-100 dark:hover:bg-yellow-900/30 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                      ...
                      <div className="space-y-3">
                        <div className={`flex items-center justify-center gap-2 px-4 py-2 rounded-lg ${
                          result.user_feedback === 'dislike' ? 'bg-red-500 text-white' :
                          result.user_feedback === 'like' ? 'bg-green-500 text-white' :
                          'bg-yellow-500 text-white'
                        }`}>
                          {result.user_feedback === 'dislike' && <><ThumbsDown className="w-5 h-5" /> Disliked</>}
                          {result.user_feedback === 'like' && <><ThumbsUp className="w-5 h-5" /> Liked</>}
                          {result.user_feedback === 'super_like' && <><Star className="w-5 h-5" /> Super Liked</>}
                        </div>
                        <button
                          onClick={() => resultsApi.removeFeedback(result.id).then(() => {
                            setResults((prev) => prev.map((r) => r.id === result.id ? {...r, user_feedback: null, feedback_at: null} : r));
                          })}
                          className="w-full px-4 py-2 text-sm rounded-lg transition-colors bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600"
                        >
                          Revert Feedback
                        </button>
